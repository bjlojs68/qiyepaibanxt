...
//             对temporaryClassesList进行遍历，根据第一个时间段的开始时间作为第一个班次的开始时间，再对
            ...
            for (i = 0; i < temporaryClassesList.size(); i++) {
                ...
                int ab=4;
                if (startTime.getHour()==10||startTime.getHour()==16){
                    ab=3;
                }else if (startTime.getHour()==11||startTime.getHour()==17){
                    ab=2;
                }
                endTime = startTime.plusHours(ab);
                for (j = i; j < temporaryClassesList.size(); j++) {
                    if (((temporaryClassesList.get(j).getStartTime().getHour() - startTime.getHour()) < ab) && temporaryClassesList.get(j).getStartTime().getHour() != startTime.getHour()) {
                        ...
                        if (employeeNumberT >= employeeNumberClasses || ((temporaryClassesList.get(j).getStartTime().getHour() - startTime.getHour()) < 2)) {
                            num++;
                            if (temporaryClassesList.size() - i > 2 && num == ab-1) {
                                Classes classes = new Classes();
