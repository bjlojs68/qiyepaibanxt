...
public class demo01 {
     ...
     *
     */
    public int[] random(int numberLength, String[] employeeArray,int[] selectedNumsB,boolean b,int[] chooseJob) {  //selectedNumsB 上一个被选中的数  b是否可以和上一个数重复
        int[] nums;
...
/*                List<Integer> chooseJob=new ArrayList<>();
                ...
                int[] random;
                if (lists.isEmpty()){
                    random = random(number, employeeArray,null,b,null);  //随机员工
                }else {
                    random = random(number, employeeArray,lists.get(i-1),b,null);  //随机员工
                }
                lists.add(random);
        ...
        for (int n = 0; n < aList.size(); n++) {
            ...
            System.out.println("员工实际工作时长数/总工作时长="+time);
            System.out.println("标准差="+"165.184");
