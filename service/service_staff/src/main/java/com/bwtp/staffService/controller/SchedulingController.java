...
//        }
        ...
        String[] timeB = new String[list2.size()];
...
//        }
        ...
        for (int i = 0; i < names.size(); i++) {
            ...
            int[] isHave = new int[timeB.length];
            for (int j = 0; j < scheduleVos.size(); j++) {
                if (dayNameHour.getEmployeeName().equals(scheduleVos.get(j).getEmployeeName())) {
                    ...
                    for (int k = 0; k < timeB.length; k++) {
                        //切割转localTime数据类型
                        String[] timeArray = timeB[k].split("-");
        ...
        return R.ok().data("total", size).data("times", timeB).data("dayNameHours", dayNameHours).data("names", names).data("list", list);
    }
...
//            System.out.println(newDate); // 输出转换后的Date对象
        ...
        for (int i = 0; i < dayNameHours.size(); i++) {
            ...
            if (!newStr.equals(oldStr)){
                String s="";
                if (numDown==0){
                    s=newStr+rosterString.substring(numUp,rosterString.length());
                }else {
                    s=rosterString.substring(0,numDown)+newStr+rosterString.substring(numUp,rosterString.length());
                }


                Scheduling scheduling = schedulingService.getOne(schedulingQueryWrapper).setRosterString(s);
                //更新数据
